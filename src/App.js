...
const k = imagesList[Math.ceil(Math.random() * 30)]
...
class App extends Component {
  ...
  sendingInfpImg = id => {
    ...
    if (imgId === id) {
      ...
      //   const j = Math.ceil(Math.random() * 30)
      const l = imagesList[Math.ceil(Math.random() * 30)]
  ...
  render() {
    const {imageUrl, tab, score, time, isGameOver} = this.state
    ...
    if (isGameOver || time === 0) {
      isGameOverOrNot = (
        <div className="game-over-score-card-container">
          <img
          ...
          <h1 className="score-result-head">{score}</h1>
          <button
          ...
          <li>
            <div className="score-time-container">
              <p className="score-head">
                Score:<span className="score-change">{score}</span>
              </p>
              <div className="timer-container">
                <img
                  ...
                  className="timer"
                />
                <p className="timer-head">{time} sec</p>
              </div>
            </div>
          </li>
        </ul>

        <div className="bg-container">{isGameOverOrNot}</div>
      </div>
    )
  }
}
